...
class PartneredSchools extends Component {
    ...
    colorCurrentFirstLetter=()=>{
        let colorImmediateCurrentFirstLetter=()=>{
            if(document.getElementsByClassName('partneredschools_letter_for_cards')[0]){
                let highestLetter=document.getElementsByClassName('partneredschools_letter_for_cards')[0].innerHTML;
                let letters = document.getElementsByClassName("partneredschools_letter");
                Array.prototype.forEach.call(letters, (el)=>{
                    if(el.innerHTML==highestLetter){
                        el.style.color="#bd1a41";
                    } else {
                        el.style.color="#a9a6ba";
                    }
        ...
        setTimeout(() => {
            colorImmediateCurrentFirstLetter();
        }, 20);
    };

    showBasedOnCountry=()=>{
        let continentsSelect=document.getElementById("continentsSelect");
        let continent = continentsSelect.options[continentsSelect.selectedIndex].value;
        
        let countriesSelect=document.getElementById("countriesSelect");
        let country = countriesSelect.options[countriesSelect.selectedIndex].value;        
                    ...
                    <div className="partneredschools_cards_wrapper clearfix">
                        ...
                        <div id="partneredschools_selects">
                            ...
                            <select onClick={()=>{this.showBasedOnContinent()}} id="continentsSelect" className="clearfix">
                                <option value="all" selected style={{backgroundColor:'#95bef0'}}>Continent</option>
                            ...
                            <select onClick={()=>{this.showBasedOnCountry()}} id="countriesSelect" className="clearfix">
                                <option value="all" selected style={{backgroundColor:'#95bef0'}}>Country</option>
                        ...
                        if(SchoolsForThisLetter.length>0)
                            ...
                            <ScrollableAnchor id={letter}>
                                <div className="partneredschools_letter_and_cards_wrapper" key={letter}>
                                    <div>
                                        <a id="partneredschools_letter_for_cards" class="partneredschools_letter_for_cards">{letter.toUpperCase()}</a>
                                    </div>
                                    <br/>
                                        ...
                                        {SchoolsForThisLetter.map((school)=>{
                                            ...
                                            <div id="partneredschools_card" key={school.id} className="clearfix">
                                                ...
                                                <h4 id="partneredschools_card_city_country">{school.country.toUpperCase()},{" "}{school.continent}</h4>
